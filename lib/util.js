...
module.exports = {
	...
	async snapPayout (dbo, txnId, contractAddress, mineable, challengeNumber) {
		...
		let docs = await this.blockShares(dbo, challengeNumber)
		let payouts = []
		docs.forEach((doc) => {
	...
	validate(challenge, publicKey, nonce, difficulty) {
		console.log(challenge, publicKey, nonce, difficulty)
	...
	async validateBlock(mineable, contractAddress, publicKey, nonce) {
		var difficulty = await mineable.getDifficulty(contractAddress)
		var challenge = await mineable.getChallengeNumber(contractAddress)
		return this.validate(challenge, publicKey, nonce, difficulty)
	},
