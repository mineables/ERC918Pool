...
module.exports = {
	async init(web3) {
		...
		this.addresses = await addr.addresses()
	...
	async getVirtualDifficulty(origin, mineableAddress)
	{
		...
		let vrig = new this.web3.eth.Contract(vrigJson.abi, this.addresses.VRIG)
	...
	async sharesCurrent(dbo, mineable, contractAddress) {

		//let mineable = new this.web3.eth.Contract(mineableJson.abi, contractAddress)
		var challenge = await mineable.getChallengeNumber(contractAddress)

		return await this.sharesForChallenge(dbo, challenge)
	},

	async sharesForChallenge(dbo, challengeNumber) {
	...
	async snapPayout (dbo, txnId, contractAddress, mineable, challengeNumber) {
		...
		let docs = await this.sharesForChallenge(dbo, challengeNumber)
		let payouts = []
	...
	async processPayouts(dbo, poolaccount, mineable) {
		...
		for (var i = 0; i < res.length; i++) {
			...
			await dbo.collection('payouts').updateMany(
		      { account: e._id.account, contract: e._id.contract, payoutTxn: { $exists: false } },
	...
	async processPayoutSingle(dbo, poolaccount, mineable, origin) {
		...
		for (var i = 0; i < res.length; i++) {
			...
			console.log('Processed payout: ' + payoutTxn, e._id.account, e.payout, e._id.contract)
			await dbo.collection('payouts').updateMany(
		      { account: origin, contract: e._id.contract, payoutTxn: { $exists: false } },
		      { $set: { 'payoutTxn' : payoutTxn } }
		    )
		}
	},
