...
var tasks = new Queue(async function (input, cb) {
    ...
    var txCount = await web3.eth.getTransactionCount(account.address) //this.seedTxnCount++
...
module.exports = {
	...
	async transfer( account, to, amount, mineableAddress, cb ) {
    	...
    	return this.sendTransaction( account, mineableAddress, method, cb)
    },
    ...
    sendTransaction(account, mineableAddress, method, cb) {
        tasks.push({ web3: this.web3, account: account, mineableAddress: mineableAddress, method: method }, cb)
    },
    ...
    async sendTransaction(account, mineableAddress, method) {
    	...
    	let encodedABI = await method.encodeABI()
    	let estimatedGas = await method.estimateGas({from: account.address})
    	var txCount = this.web3.util.randomHex(32) // await this.web3.eth.getTransactionCount(account.address) //this.seedTxnCount++
